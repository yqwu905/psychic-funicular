...
已实现：`有效优先级 = 显式优先级 + age_weight × 排队分钟数`（M5：基础 + 老化，`age_weight` 可配，默认 0）。

后期多因子：再加 `fairshare`（历史用量越多优先级越低）、`jobsize`、`qos` 加权求和 ——
与 Slurm `priority/multifactor` 一致的思路。
...
EASY backfill（单预留）：遇到最高优先级但暂时排不上的作业时，按当前运行作业的
`start + walltime` 估算它在各节点上的**最早可起始时间**作为预留；之后只允许「有界 walltime
且能在该预留时间前结束」的低优先级作业插队，从而既不饿死大作业、又用短作业填满空隙。
预留估算偏保守（绝不推迟被预留作业）。开关：`scheduler.backfill`（默认开）。
