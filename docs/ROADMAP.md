...
- ✅ **已演示**：真实 sshd 跑在 2222，节点经隧道注册并在其中跑作业——注册/指标/调度/执行/日志
  全链路走在一条 SSH 连接里。
- ✅ **Agent 自举（M3.1）**：`provision: true` 时控制平面在隧道就绪后经 SCP 分发 agent 并远程拉起
  （nohup + pid 落盘 + 退避重试），免去手动 `scp` + 启动。
- ✅ **失联诊断（M3.1）**：节点失联持续一段时间后经 SSH 诊断根因——区分「SSH 连接中断 /
  agent 进程被杀 / 其他原因」，发 `node.diagnosed` 事件；可选 `auto_restart` 自动重拉。
